...
pub fn get_metadata(video_path: &Path) -> Result<VideoMetadata> {
    ...
    let output = std::process::Command::new("ffprobe")
        ...
        .map_err(|e| {
            WatchError::Ffmpeg(format!(
                "ffprobe not found or failed to execute for '{}': {}",
                video_str, e
            ))
        })?;
    ...
    let json: serde_json::Value = serde_json::from_slice(&output.stdout).map_err(|e| {
        ...
        let snippet = if preview.len() > 200 {
            &preview[..200]
        } else {
            &preview
        };
    ...
    if duration < 1.0 {
        eprintln!(
            "[watch2] warning: very short video ({:.2}s), frame extraction may produce few or no frames",
            duration
        );
    }
