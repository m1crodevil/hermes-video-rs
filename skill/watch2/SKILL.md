...
**NEVER use Python for JSON manipulation.** The `key_moments.json` schema is defined in `moments.rs` — Rust parses it natively. If the JSON is invalid, fix the JSON content (not the parser). Common mistakes:
- Using float seconds instead of MM:SS string for `timestamp`
- Including `timestamp_fmt` (auto-generated by Rust)
- Including `frame_path` (set by Rust after extraction)
...
**key_moments.json SCHEMA (MANDATORY — read before writing):**
```json
[
  {
    "timestamp": "0:54",           // MM:SS format (string, NOT float seconds)
    "word": "Ragnarok",            // triggering word/phrase
    "context": "Ya kan Ragnarok...", // 1-2 sentences around this moment
    "reason": "proper_noun",       // one of: proper_noun, claim, deictic, speaker_id, visual_context, entity, topic_transition, key_argument
    "question": "What game name is displayed on screen?", // specific vision question
    "priority": 1                  // 1 (critical) to 5 (nice-to-have)
  }
]
```

**RULES for key_moments.json:**
- `timestamp` MUST be MM:SS string (e.g., "0:54", "10:30", "1:05:20")
- `timestamp` MUST NOT be float seconds (e.g., 54.0 is WRONG)
- Do NOT include `timestamp_fmt` — Rust auto-generates it
- Do NOT include `frame_path` — Rust sets it after extraction
- `reason` MUST be one of the 8 valid values listed above
